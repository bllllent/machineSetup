...
| `shift-utc-times.py [--apply]` | Fixes UTC wall-clock times left over from importing under a UTC server: reinterprets the stored instant in Pacific time, per-asset DST-correct. Only touches assets with no date on disk and no prior timezone correction — EXIF-dated assets are never shifted. DB-only; flush with the sync afterwards. |
